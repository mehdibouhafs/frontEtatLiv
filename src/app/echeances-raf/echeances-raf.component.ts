...
@Component({
  selector: 'app-echeances-raf',
  templateUrl: './echeances-raf.component.html',
  styleUrls: ['./echeances-raf.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EcheancesRafComponent implements  OnInit,OnChanges,OnDestroy {
  ...
  getEcheancesNotLinkedDelay(date:string,nameModele:string,page :number,size:number,sortBy:any,sortType:any){


    this.contratService.getEcheancesNotLinkedDelay(date,nameModele,page,size,sortBy,sortType).subscribe(
      (data : any)=>{
  ...
  loadEcheances(){
    ...
    this.getEcheancesNotLinkedDelay(date,this.modeleSelected,this.currentPageEcheances,this.pageSizeEcheances,this.sortBy,this.sortType);
  ...
  composeEmail(){
    ...
    var email="mailto:?subject= EchéancesRAF selectionnées :"+ "&body= Bonjour,%0A"
      +"Ce message concerne les échéancesRAF séléctionées dont le détail est ci-après :"+"%0A"+"%0A"
  ...
  exportEcheancesNotLinkedDelay($event){
    ...
    var result= this.contratService.exportEcheancesNotLinkedDelay(date);

    var d = new Date();

    //console.log("day " + d.getDay());
    var fileName = "EcheancesRAF-"+moment(new Date()).format("DD-MM-YYYY")+"-"+d.getHours()+"-"+d.getMinutes()+".xlsx";
